...
class TestClaimTask:
    def test_claim_pending(self, tmp_queue_dir):
        ...
        claimed = claim_task(task["id"], "node_gpu-123")
        assert claimed is not None
        assert claimed["state"] == "claimed"
        assert claimed["claimed_by"] == "node_gpu-123"

    def test_claim_already_claimed(self, tmp_queue_dir):
        from orchestrator import create_task, claim_task

        task = create_task("Taken", "/opt/test")
        claim_task(task["id"], "node_gpu-123")
        second = claim_task(task["id"], "node_primary-456")
