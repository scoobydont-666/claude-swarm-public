...
class TestRegister:
    def test_register_creates_task(self, tmp_registry):
        task = tmp_registry.register(
            "dispatch-001", host="gpu-server-1", description="test task"
        )
        assert task.dispatch_id == "dispatch-001"
        assert task.host == "gpu-server-1"
        assert task.status == "running"

    def test_register_persists_to_disk(self, tmp_registry):
        tmp_registry.register("dispatch-001", host="gpu-server-1")
    ...
    def test_register_with_task_id(self, tmp_registry):
        task = tmp_registry.register("dispatch-001", host="gpu-server-1", task_id="task-042")
    ...
    def test_register_reads_pid_from_file(self, tmp_registry, dispatch_dir):
        ...
        task = tmp_registry.register("dispatch-001", host="gpu-server-1")
        assert task.pid == 12345

    def test_register_with_explicit_pid(self, tmp_registry):
        task = tmp_registry.register("dispatch-001", host="gpu-server-1", pid=9999)
        assert task.pid == 9999


class TestActive:
    def test_active_returns_running_only(self, tmp_registry):
        tmp_registry.register("d-1", host="gpu-server-1")
        tmp_registry.register("d-2", host="gpu-server-2")
...
class TestPoll:
    def test_poll_detects_dead_process(self, tmp_registry):
        task = tmp_registry.register("d-1", host="gpu-server-1", pid=99999999)
        completed = tmp_registry.poll()
        assert len(completed) == 1
        assert completed[0].status == "completed"

    def test_poll_ignores_live_process(self, tmp_registry):
        task = tmp_registry.register("d-1", host="gpu-server-1", pid=os.getpid())
        completed = tmp_registry.poll()
    ...
    def test_poll_updates_dispatch_record(self, tmp_registry, dispatch_dir):
        ...
        tmp_registry.register("d-1", host="gpu-server-1", pid=99999999)
        tmp_registry.poll()
    ...
    def test_poll_reads_pid_lazily(self, tmp_registry, dispatch_dir):
        tmp_registry.register("d-1", host="gpu-server-1", pid=-1)
...
class TestSummary:
    ...
    def test_summary_active_hosts(self, tmp_registry):
        ...
        tmp_registry.register("d-2", host="gpu-server-2")
        s = tmp_registry.summary()
        assert set(s["active_hosts"]) == {"gpu-server-1", "gpu-server-2"}
...
class TestPersistence:
    def test_reload_from_disk(self, tmp_path):
        ...
        reg1.register("d-1", host="gpu-server-1", description="test")
