...
class TestAgentRegistry:
    def test_register_and_list(self):
        redis_client.register_agent("gpu-server-1", 1234, {"gpu": True})
        agents = redis_client.list_agents()
        assert len(agents) == 1
        assert agents[0]["host"] == "gpu-server-1"
        assert agents[0]["pid"] == "1234"

    def test_heartbeat_refreshes(self):
        redis_client.register_agent("gpu-server-1", 1234)
        assert redis_client.heartbeat("gpu-server-1", 1234)

    def test_heartbeat_unregistered_returns_false(self):
        assert not redis_client.heartbeat("gpu-server-1", 9999)

    def test_unregister(self):
        redis_client.register_agent("gpu-server-1", 1234)
        redis_client.unregister_agent("gpu-server-1", 1234)
        agents = redis_client.list_agents()
        assert len(agents) == 0
...
class TestMessaging:
    def test_send_and_read(self):
        redis_client.send_message("gpu-server-1", {"action": "sync"})
        msgs = redis_client.read_inbox("gpu-server-1")
        assert len(msgs) == 1
        assert msgs[0]["action"] == "sync"

    def test_read_pop(self):
        redis_client.send_message("gpu-server-1", {"action": "sync"})
        msgs = redis_client.read_inbox("gpu-server-1", pop=True)
        assert len(msgs) == 1
        # Inbox should be empty after pop
        assert redis_client.read_inbox("gpu-server-1") == []
...
class TestStatus:
    def test_update_and_get(self):
        redis_client.update_status("gpu-server-1", {"load": 0.5, "gpu_free": True})
        status = redis_client.get_status("gpu-server-1")
