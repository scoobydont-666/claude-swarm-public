...
class TestCreateWorktree:
    ...
    def test_sanitizes_dispatch_id(self, git_repo, worktree_base):
        wt = create_worktree(git_repo, "dispatch/with spaces/special", host="localhost", worktree_base=worktree_base)
        assert wt is not None
...
class TestMergeWorktree:
    def test_merge_with_changes(self, git_repo, worktree_base):
        ...
        subprocess.run(["git", "-C", wt.path, "commit", "-m", "worktree change"], capture_output=True)
        # Merge back
    ...
    def test_merge_no_changes(self, git_repo, worktree_base):
        wt = create_worktree(git_repo, "test-no-change", host="localhost", worktree_base=worktree_base)
        assert wt is not None
...
class TestCleanup:
    def test_cleanup_removes_worktree(self, git_repo, worktree_base):
        wt = create_worktree(git_repo, "test-cleanup", host="localhost", worktree_base=worktree_base)
        assert wt is not None
        assert Path(wt.path).exists()
