...
STUDENT_TRAIN = Pipeline(
    name="student-train",
    description="Train a student model via QLoRA on gpu-server-1, with GPU lifecycle management",
    stages=[
        PipelineStage(
            name="claim_gpu",
            role="Stop vLLM on gpu-server-1 to free GPU for training",
            model="haiku",
            host="giga",
            requires=["gpu"],
            depends_on=[],
            prompt_template="""Prepare gpu-server-1 GPU for training by stopping vLLM.
Run:
...
Report: VRAM status before and after.""",
        ...
        PipelineStage(
            ...
            prompt_template="""Train student model on gpu-server-1.
Run: cd /opt/ntnx-codeforge && python3 finetune.py \
  --model-size {input.model_size} \
  --dataset {input.dataset_path} \
  --run-id {input.run_id} \
  --output-dir /var/lib/swarm/training/checkpoints/{input.run_id}
...
Report: pass/fail for each test, overall verdict.""",
        ...
        PipelineStage(
            name="release_gpu",
            role="Restart vLLM on gpu-server-1 after training",
            model="haiku",
            host="giga",
            requires=[],
            depends_on=["smoke_test"],
            prompt_template="""Restore vLLM on gpu-server-1 after training completes.
Run:
