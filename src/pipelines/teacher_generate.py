...
Run: cd /opt/ntnx-codeforge && python3 scripts/vllm_batch_generate.py \
  ...
  --output /opt/swarm/training/datasets/generated-{input.run_id}-giga.jsonl \
...
Run: cd /opt/ntnx-codeforge && python3 scripts/vllm_batch_generate.py \
  ...
  --output /opt/swarm/training/datasets/generated-{input.run_id}-mega.jsonl \
...
giga = [json.loads(l) for l in open('/opt/swarm/training/datasets/generated-{input.run_id}-giga.jsonl')]
mega = [json.loads(l) for l in open('/opt/swarm/training/datasets/generated-{input.run_id}-mega.jsonl')]
combined = giga + mega
...
path = '/opt/swarm/training/datasets/teacher-corpus-{input.run_id}.jsonl'
with open(path, 'w') as f:
