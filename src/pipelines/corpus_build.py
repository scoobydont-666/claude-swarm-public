...
Run: cd /opt/ntnx-codeforge && python3 data/extract_code.py --output /opt/swarm/training/datasets/raw-{input.run_id}.jsonl
...
import json
records = [json.loads(l) for l in open('/opt/swarm/training/datasets/raw-{input.run_id}.jsonl')]
clean, audit = sanitize_batch(records)
with open('/opt/swarm/training/datasets/sanitized-{input.run_id}.jsonl', 'w') as f:
...
import json
records = [json.loads(l) for l in open('/opt/swarm/training/datasets/sanitized-{input.run_id}.jsonl')]
unique, exact_removed = dedup_exact(records)
final, near_removed = dedup_near(unique, threshold=0.8)
with open('/opt/swarm/training/datasets/deduped-{input.run_id}.jsonl', 'w') as f:
...
import json
path = '/opt/swarm/training/datasets/deduped-{input.run_id}.jsonl'
...
m.to_yaml(f'/opt/swarm/training/datasets/{{sha}}/manifest.yaml')
